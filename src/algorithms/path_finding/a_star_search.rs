...
use crate::{components::custom_components::matrix_component::draw_matrix, models::cell::Cell};
use crate::{components::store::matrix_state::MatrixState, models::matrix::Matrix};
...
pub fn a_star_search(matrix_dispatch: &Dispatch<MatrixState>) -> Option<i32> {
    ...
    while !frontier.is_empty() {
        ...
        for neighbour in get_neighbours(&matrix, coords) {
                ...
                || new_cost < *cost_so_far.entry(neighbour).or_insert(f64::INFINITY)
            {
                ...
                frontier.push(Node {
                    coords: neighbour,
                    cost: new_cost + manhattan_distance(neighbour, end),
                });
